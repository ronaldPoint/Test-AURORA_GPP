...
class VisibilityRoadMap:
    ...
    def planning(self, start_x, start_y, goal_x, goal_y, obstacles):
        ...
        rx, ry, cnt1 = DijkstraSearch(show_animation).search(
            start_x, start_y,
            goal_x, goal_y,
...
def main():
    ...
    algorithm = 'Visibility_Road_Map'
    setup = 0
    # cases = [1, 2, 3, 4, 5, 6,7]
    for c in range (1,8):
        coord, map = map_type.clearance(c)  
        obstacles, sx, sy, gx, gy, grid_size, robot_radius = scenarios.scenario_XB(coord)

#    obstacles, map = Maps.get_map_P1B_empty()   # Map1, Payam Squares
#    obstacles, map = Maps.get_map_P1B() # Payam Version Map1
#    obstacles, map = Maps.get_map_P2B() # Payam Version Map2
    # obstacles, map = Maps.get_map_P3B() # Payam Version Map3

    # obstacles, map = Maps.get_map_C5B() # Payam Version Map3
...
#    sx, sy, gx, gy, grid_size, robot_radius = Maps.init_P4()
        ...
        rx, ry, iter = VisibilityRoadMap(robot_radius, do_plot=show_animation).planning(
